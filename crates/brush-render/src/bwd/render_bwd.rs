...
fn use_unchecked_raster_bwd() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        let enabled = crate::native_msl::option_requested(
            crate::native_msl::UNCHECKED_RASTER_BWD_ENV,
        );
        if enabled {
            tracing::warn!(
...
fn use_coalesced_sh_grad() -> bool {
    ...
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        crate::native_msl::option_requested(crate::native_msl::COALESCED_SH_GRAD_ENV)
    })
}
